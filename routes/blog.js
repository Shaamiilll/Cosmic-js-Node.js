...
module.exports = (app, config, bucket, partials, _) => {
  app.get('/blog', async (req, res) => {
    try {
      ...
      return res.status(500).send({ "status": "error", "message": "Yikes, something went wrongs!" })
    }
  })
  app.get('/blog/:slug', async (req, res) => {
    ...
    try {
      ...
      console.error("🔥 Blog Route Error:", error); // <-- Log the actual error
      return res.status(500).json({ "status": "error", "message": error.message || "Yikes, something went wrong!" });
    }
  })
}
